...
const Chat = () => {
  ...
  return (
    <div className="w-3/4 lg:w-1/2 mx-auto border border-gray-600 m-5 h-[80vh] flex flex-col">
      <h1 className="p-5 border-b border-gray-600"> Chat
        ...
        {messages.map((msg, index) => {
          return (
            <div key={index} className={`chat ${user.firstName === msg.firstName ? "chat-end" : "chat-start"}`}>
              ...
              <div className={`chat-bubble ${user.firstName === msg.firstName ? "bg-primary text-white" : ""}`}>{msg.text}</div>
      ...
      <div className="p-5 border-t border-gray-600 flex items-center gap-2">
        ...
        onChange={(e) => setNewMessages(e.target.value)}
          className="flex-1 w-auto border border-gray-600 text-white rounded p-2"
          type="text"
        />
